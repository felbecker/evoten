...
Args:
    ...
    leaf_indices: Indices of leaves in the tree of shape (num_leaves). Can be obtained by tree_handler.get_leaf_indices(leaf_names).
...
Returns:
    ...
    else shape (num_ancestral_nodes, L, length, models, d)
"""
@backend.decorator
def compute_ancestral_probabilities(leaves, leaf_names,
    ...
    for depth in range(tree_handler.depth, 0, -1):
        ...
        num_nodes_above = tree_handler.layer_sizes[depth-1]
        num_leaves_above = tree_handler.layer_leaves[depth-1]

        # aggregate over child nodes
        child_counts = tree_handler.get_child_counts_by_depth(depth-1)
        X_anc = backend.aggregate_children_log_probs(T, child_counts)
        if depth > 1:
            X_leaves = leaves[:, proc_leaves:proc_leaves+num_nodes_above]
            proc_leaves += num_nodes_above
            X = backend.concat([X_leaves, X_anc])
            
        if not return_only_root:
            anc_probs = anc_probs.write(depth - tree_handler.depth, X_anc)
...
Args:
    leaves: Logits of all symbols at all leaves of shape (num_leaves, k, models, d).
    tree_handler: TreeHandler object
    rate_matrix: Rate matrix of shape (models, d, d)
