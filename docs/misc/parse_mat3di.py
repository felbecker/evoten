...
def parse_mat3di(path: Path | str) -> tuple[np.ndarray, np.ndarray, list[str]]:
    ...
    Returns:
        A tuple ``(odds_ratios, background, alphabet)`` where
        ``odds_ratios`` has shape ``(n, n)`` and contains p(xy)/p(x)p(y),
        ``background`` shape ``(n,)`` and contains p(x), and ``alphabet``
        is a list of *n* single-letter labels (the 'X' wildcard row/column is
        omitted).
    """
    with open(path) as f:
        lines = f.readlines()
    ...
    background = background[orig_indices]
    background = (background / background.sum()).astype(np.float32)
    ...
    odds_ratios = np.exp(lambda_val * score_matrix).astype(np.float32)

    return odds_ratios, background, alphabet_clean
