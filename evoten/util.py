...
def encode_tuple_alignment(ta, k=3, gap_symbols='-'):
    """
    ...
    are encoded as all-zeros (unknown/missing).
    ...
    Returns:
        np.ndarray: Shape [R, L, 4**k], dtype float32. Valid k-mer entries
            are one-hot vectors. Gap and ambiguous entries are all-ones
...
def tuple_array(sequences, k=3, gap_symbols='-'):
    """
    ...
    indexing. Gap and ambiguous entries remain as all-ones.
    ...
    Returns:
        np.ndarray: Shape [R, L, 4**k], dtype float32.
    """
    ...
    R = len(sequences)
    L = len(valid_tuples)
    result = np.ones((R, L, 4 ** k), dtype=default_dtype)

    # Step 3: fill one-hot entries using batch numpy indexing per row
    ...
    for r, d in enumerate(seq_dicts):
        ...
        for t, base_idx in d.items():
            ...
            if j is not None and base_idx >= 0:
                js.append(j)
                idxs.append(base_idx)
        if js:
            js = np.array(js, dtype=np.intp)
            idxs = np.array(idxs, dtype=np.intp)
            result[r, js, :] = 0.0
            result[r, js, idxs] = 1.0
